...
The CUDA path uses a CuTe DSL fused panel kernel and a raw CUDA tiled WY trailing update.
"""
...
void apply_panel_wy_tiled_cuda(torch::Tensor h, torch::Tensor tmat, int64_t j_start, int64_t j_end) {
  ...
  if (active_rows <= FUSED_ROWS) {
    ...
    apply_panel_wy_fused_tile_kernel<<<grid_fused, block_fused, 0, stream>>>(
        h.data_ptr<float>(),
        tmat.data_ptr<float>(),
        batch,
        m,
  ...
  compute_y_partial_kernel<<<grid1, block1, 0, stream>>>(
      h.data_ptr<float>(),
  ...
  reduce_y_compute_z_kernel<<<grid2, block2, 0, stream>>>(
      y_partial.data_ptr<float>(),
      tmat.data_ptr<float>(),
      z.data_ptr<float>(),
      batch,
  ...
  update_c_tiled_kernel<<<grid3, block3, 0, stream>>>(
      h.data_ptr<float>(),
      z.data_ptr<float>(),
      batch,
      m,
